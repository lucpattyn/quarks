...
class Routing{
	...
	class Router{
		...
		private:
		std::map< std::string , 
			std::function<std::string(Routing::Request& request)> > routingTableString;
		std::map< std::string , 
			std::function<crow::json::wvalue(Routing::Request& request)> > routingTableJson;
	};
	
public:
	static Router& DefaultRouter();
	static void BuildRoutes(Routing::Router& router);
