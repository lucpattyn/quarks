...
#define ROUTE(ROUTER, URL) (ROUTER[URL]) = ROUTING_CALLBACK
#define ROUTING_CALLBACK(c) c

#define ROUTING_REQ_UNUSED(u) u.query.size()
...
/*std::map< std::string , 
		std::function<std::string(Routing::Request& request)> > 
	Routing::Router::routingTableString;

std::map< std::string , 
		std::function<crow::json::wvalue&(Routing::Request& request)> >
	Routing::Router::routingTableJson;*/

Routing::Router& Routing::DefaultRouter(){
	static Routing::Router defaultRouter;
	return defaultRouter;
}

// Route mapping starts ...
	
void Routing::BuildRoutes(Routing::Router& router){

	auto route_ping_callback = 
	[](const Routing::Request& /*req*/) {
		return "pong";  
	};
...
///////////////////////////////////////////
	...
	[post](const Routing::Request& req) {
	...
	[putatom](const Routing::Request& req) {
	...
	[removeatom](const Routing::Request& req) {
	...
	[atom](const Routing::Request& req) {
	...
	ROUTE(router, "/getsorted")
	(route_core_getsorted_callback);

	ROUTE(router, "/getkeys")
	(route_core_getkeys_callback);

	ROUTE(router, "/getkeysmulti")
	(route_core_getkeysmulti_callback);

	ROUTE(router, "/getcount")
	...
	// experimental
	ROUTE(router, "/filetransfer")
	(route_core_filetransfer_callback);

	ROUTE(router, "/opentcpsocket")
...
#ifdef _USE_PLUGINS
	ROUTE(router, "/filter/gaussian")
	(route_filter_gaussian_callback);

	ROUTE(router, "/filter/adjust")
	(route_filter_adjust_callback);

	ROUTE(router, "/filter/clahe")
...
//
std::string Routing::DispatchRequest(const char* data){
	return Routing::DefaultRouter().dispatch(data);
}
